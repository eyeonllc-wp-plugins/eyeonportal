...
(function( window, document, undefined ) {
	...
	(function( factory ) {
		...
		function QTip( target, options, id, attr ) {
			...
			// Cache object
			this.cache = {
				event: {}, target: $(), disabled: FALSE, attr: attr, onTooltip: FALSE, lastClass: ''
			};
		...
		PROTOTYPE.reposition = function( event, effect ) {
			...
			// Target wasn't mouse or absolute...
				...
				// Check if window or document is the target
					...
					if ( target[0] === window ) {
						position = {
							top: (viewport || target).scrollTop(), left: (viewport || target).scrollLeft()
						};
					}
				}
		...
		;PROTOTYPE.toggle = function( state, event ) {
			...
			after = $.proxy( function() {
				if ( state ) {
					...
					// Reset CSS states
					tooltip.css( {
						display: '', visibility: '', opacity: '', left: '', top: ''
					} );
				}
		...
		;PROTOTYPE._createButton = function() {
			...
			if ( button.jquery ) {
				...
				elements.button = $( '<a />', {
					...
					'class': 'ui-icon ui-icon-close', 'html': '&times;'
				} ) );
			}
		...
		QTIP.defaults = {
			prerender: FALSE, id: FALSE, overwrite: TRUE, suppress: TRUE, content: {
				text: TRUE, attr: 'title', title: FALSE, button: FALSE
			}, position: {
				my: 'top left', at: 'bottom right', target: FALSE, container: FALSE, viewport: FALSE, adjust: {
					x: 0, y: 0, mouse: TRUE, scroll: TRUE, resize: TRUE, method: 'flipinvert flipinvert'
				...
				target: FALSE, event: 'mouseenter', effect: TRUE, delay: 90, solo: FALSE, ready: FALSE, autofocus: FALSE
			}, hide: {
				target: FALSE,
				event: 'mouseleave',
				effect: TRUE,
				...
				classes: '', widget: FALSE, width: FALSE, height: FALSE, def: TRUE
			}, events: {
				render: NULL,
				move: NULL,
				show: NULL,
		...
		$.extend( Tip.prototype, {
			...
			update: function( corner, position ) {
				...
				tip.css( {
					width: newSize[0], height: newSize[1], lineHeight: newSize[1] + 'px'
				} );
		...
		$.extend( TRUE, QTIP.defaults, {
			style: {
				tip: {
					corner: TRUE, mimic: FALSE, width: 6, height: 6, border: TRUE, offset: 0
				}
			}
		} );
		...
		OVERLAY = function() {
			var self = this, focusableElems = {}, current, onLast, prevState, elem;
		...
		$.extend( TRUE, QTIP.defaults, {
			show: {
				modal: {
					on: FALSE, effect: TRUE, blur: TRUE, stealfocus: TRUE, escape: TRUE
				}
			}
		} );
		...
		;PLUGINS.polys = {
			...
			polygon: function( baseCoords, corner ) {
				...
				if ( corner.abbrev() === 'c' ) {
					result.position = {
						left: result.position.left + (result.width / 2), top: result.position.top + (result.height / 2)
					};
				} else {
			...
			rect: function( ax, ay, bx, by ) {
				return {
					width: Math.abs( bx - ax ), height: Math.abs( by - ay ), position: {
		...
		;PLUGINS.svg = function( api, svg, corner ) {
			...
			switch ( elem.nodeName ) {
				...
				default:
					result = elem.getBBox();
					result = {
						width: result.width, height: result.height, position: {
							left: result.x, top: result.y
						}
					};
					break;
			}
