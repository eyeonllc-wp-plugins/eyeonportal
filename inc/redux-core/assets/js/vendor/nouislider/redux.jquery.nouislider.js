...
(function( $ ) {
	...
	var noConflict = 'reduxNoUiSlider';

	if ( $['zepto'] && ! $.fn.removeData ) {
		throw new ReferenceError( 'Zepto is loaded without the data module.' );
	}

	$.fn[noConflict] = function( options, rebuild ) {

		var // Cache the document and body selectors;
		...
		function fixEvent( e ) {
			...
			var touch                                                    = e.type.indexOf( 'touch' ) === 0, mouse           = e.type.indexOf( 'mouse' ) === 0,
			    pointer                                                  = e.type.indexOf( 'pointer' ) === 0, x, y, event = e;
			...
			return $.extend( event, {
				'pointX': x, 'pointY': y, cursor: mouse
			} );
		}
		...
		function store( handle, i, serialization ) {
			...
			return {
				element: handle, elements: elements, target: handle.data( 'target' ), 'val': serialize
			};
		}
		...
		function setHandle( handle, to ) {
			...
			var base                                                                     = handle.data( 'base' ), settings = base.data( 'options' ), handles = base.data( 'handles' ),
			    lower = 0, upper = 100;
		...
		function move( event, Dt, Op ) {
			...
			var handles = Dt.handles, limits, proposal = event[Dt.point] - Dt.start[Dt.point];
		...
		function start( event, Dt, Op ) {
			...
			attach( actions.end, doc, end, {
				target: Dt.target, handles: Dt.handles
			} );
		...
		function test( input, sliders ) {
			...
			var serialization = {
				resolution: function( q, o ) {
					...
					function ser( r ) {
						return isInstance( r ) || typeof r === 'string' || typeof r === 'function' || r === false || (isInstance( r[0] ) && typeof r[0][r[1]] === 'function');
					}
			...
			$.each( tests, function( name, test ) {
				...
				if ( (test['r'] && ! isSet) || // If the test returns false, fail.
					...
					throw new RangeError( noConflict );
				}
			} );
		}
		...
		function create( options ) {
			...
			return this.each( function() {
				...
				var target = $( this ), i, dragable, handles = [], handle, base = $( '<div/>' ).appendTo( target );
