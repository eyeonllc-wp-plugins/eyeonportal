          ...
          const eventItem = $(`
            <a href="${event.event_url?event.event_url:`<?= mcd_single_page_url('mycenterevent') ?>${event.slug}`}" class="event event-${event.id}" ${(event.event_url && settings.external_event_new_tab)?'target="_blank"':''}>
              ...
              <div class="event-content">
                ...
                ${ (event.show_date || event.show_event_time) ? `
                  <div class="metadata">
                    ${ event.show_date ? `
                      <div class="date">
                        <i class="far fa-calendar"></i>
                        ${ event.show_upcoming_date ? `<span>${event.datesStr}</span>` : ``}
                        ${ event.show_date_range ? `<span>${event.formatted_start_date} - ${event.formatted_end_date}</span>` : ``}
                      </div>
                    `:``}
